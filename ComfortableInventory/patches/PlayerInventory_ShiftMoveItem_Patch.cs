        ...
        internal static void Replacement(PlayerInventory inventory, Slot slot)
        {
            ...
            for (; index < end; index++)
            {
                ...
                if (iterSlot.itemInstance?.UniqueIndex == uniqueIndex && !iterSlot.StackIsFull())
                    PatchHelpers.PlayerInventory_StackSlots(inventory, new object[] { slot, iterSlot, item.Amount });
            ...
            if (item != null && firstEmptySlot != null)
                PatchHelpers.PlayerInventory_MoveSlotToEmpty(inventory,
                    new object[] { slot, firstEmptySlot, item.Amount });
        }
